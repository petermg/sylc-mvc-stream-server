		...
		if (pthread_cond_init(&dec->task_ready, NULL) == 0) {
			if (pthread_cond_init(&dec->task_progress, NULL) == 0) {
				if (pthread_cond_init(&dec->task_complete, NULL) == 0) {
					...
					#ifdef __ANDROID__
						// Android bionic does not implement pthread_cancel. Cooperatively
						// wake and join any workers that were created before the failure.
...
int edge264_decode_NAL(Edge264Decoder *dec, const uint8_t *buf, const uint8_t *end, Edge264UnrefCb unref_cb, void *unref_arg)
{
	...
	// MVC-FIX: For MVC streams (ssps populated), each view has its own
	// get_frame_queue of up to 16 entries, so the safety margin must
	// accommodate the sum of both queues (up to 32). The original limit of 16
	// caused deadlock on MVC streams as soon as both views' queues started
	// filling: queue[0]=16 + queue[1]=N + bumpable=1 > 16 → ENOBUFS
	// even though plenty of DPB capacity remained.
	// MVC-FIX: For MVC streams (ssps populated), each view has its own
	// get_frame_queue of up to 16 entries, so the safety margin must
	// accommodate both queues (up to 32 total).
	...
	int dpb_safety_margin = dec->ssps.BitDepth_Y ? 32 : 16;
	if (queued0 + queued1 + bumpable > dpb_safety_margin) {
		if (dec->n_threads)
			pthread_mutex_unlock(&dec->lock);
		return ENOBUFS;
	}
...
int edge264_get_frame(Edge264Decoder *dec, Edge264Frame *out, int borrow) {
	...
	if (idx0 >= 0 && dec->next_deblock_addr[pic0 = dec->get_frame_queue[0][idx0]] == INT_MAX &&
		...
		if (idx1 >= 0) {
			...
			out->PictureOrderCnt_mvc = dec->FieldOrderCnt[0][pic1];
...
void edge264_release_frame(Edge264Decoder *dec, void *return_arg) {
	if (dec != NULL && return_arg != NULL)
		edge264_return_frame(dec, return_arg);
}
...
unsigned edge264_get_busy_tasks(Edge264Decoder *dec) {
	if (dec == NULL)
		return 0;
	if (dec->n_threads)
		pthread_mutex_lock(&dec->lock);
	unsigned busy = dec->busy_tasks;
	if (dec->n_threads)
		pthread_mutex_unlock(&dec->lock);
	return busy;
}

unsigned edge264_bump_and_get_busy(Edge264Decoder *dec) {
	if (dec == NULL)
		return 0;
	edge264_bump_frames(dec);
	return edge264_get_busy_tasks(dec);
}
...
int edge264_is_frame_ready(Edge264Decoder *dec) {
	...
	for (int i = 0; i < 16; i++) {
		if (dec->get_frame_queue[0][i] >= 0) {
			int pic = dec->get_frame_queue[0][i];
			if (dec->next_deblock_addr[pic] == INT_MAX) {
				ready = 1;
				break;
			}
		}
